...
`octantscope` is a terminal oscilloscope heavily modeled on [dood.al/oscilloscope](https://dood.al/oscilloscope/). It renders a real-time XY phosphor display — complete with Gaussian beam glow, phosphor persistence, and a CRT graticule. A `--sixel` flag for terminals that support it.
